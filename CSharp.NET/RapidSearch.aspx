<%@ Page language="c#" Inherits="com.qas.prowebintegration.RapidSearch" EnableEventValidation="false" CodeFile="RapidSearch.aspx.cs" %>
<%@ Import namespace="com.qas.prowebintegration" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN" >
<HTML>
	<HEAD>
		<title>QAS Pro On Demand</title>
...
%>
			...
			function submitSearch()
			{
				...
				{
					preSubmit();
					<%= GetPostBackEventReference(ActionSearch) %>;
				}
			}
			...
			function <%= Commands.StepIn %>(sMoniker, sPickText, sPostcode, iScore, sWarning)
			{
				...
				<%= GetPostBackEventReference(ActionStepIn) %>;
			}
			...
			function <%= Commands.Format %>(sMoniker, sWarning)
			{
				...
				document.getElementById("<%= HiddenMoniker.ID %>").value = sMoniker;
				document.getElementById("<%= HiddenWarning.ID %>").value = sWarning;
				<%= GetPostBackEventReference(ActionFormat) %>;
			}
	...
	<body onload="init();">
		<form id="formAction" method="post" runat="server" onsubmit="return validate(this);">
			...
			<input type=hidden id="<%=Constants.FIELD_DATA_ID %>" name="<%=Constants.FIELD_DATA_ID %>" value="" />
		</form>
	</body>
</HTML>
