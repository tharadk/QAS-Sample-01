	...
	public partial class HierSearch : HierBasePage
	{
		...
		protected void InitialSearch()
		{
			...
			try
			{
                ...
                canSearch = searchService.CanSearch(StoredDataID, GetLayout(), PromptSet.Types.OneLine);
				...
				else
				{
					...
					m_Picklist = searchService.Search(StoredDataID, StoredUserInput, PromptSet.Types.OneLine, null, RequestTag.Text).Picklist;  // throws Exception
					...
					{
						Moniker = m_Picklist.Items[0].Moniker;
						bStepPastClose  = bStepPastClose  || m_Picklist.IsAutoStepinPastClose;
						bCrossBorder    = bCrossBorder	 || m_Picklist.Items[0].IsCrossBorderMatch;
						bPostcodeRecode = bPostcodeRecode || m_Picklist.Items[0].IsPostcodeRecoded;
						// Add this step-in to history
						m_aHistory.Push(m_Picklist.Items[0]);
						// Auto-step into first item
						m_Picklist = searchService.StepIn(Moniker);  // throws Exception
					}
		...
		protected void RefinementSearch()
		{
			...
			try
			{
				...
				m_Picklist = searchService.Refine(Moniker, RefineText + " ", RequestTag.Text);	// throws Exception

				// Auto-step-in logic - if a solo step-in item
				if (m_Picklist.IsAutoStepinSingle)
				{
					Moniker = m_Picklist.Items[0].Moniker;
					bCrossBorder    = bCrossBorder    || m_Picklist.Items[0].IsCrossBorderMatch;
					bPostcodeRecode = bPostcodeRecode || m_Picklist.Items[0].IsPostcodeRecoded;
				...
				else if (m_Picklist.IsAutoFormatSingle)
				{
					Moniker = m_Picklist.Items[0].Moniker;
					bCrossBorder    = bCrossBorder    || m_Picklist.Items[0].IsCrossBorderMatch;
					bPostcodeRecode = bPostcodeRecode || m_Picklist.Items[0].IsPostcodeRecoded;
		...
		protected void RenderPicklist()
		{
			...
			for (int iIndex = 0; iIndex < m_Picklist.Items.Length; ++iIndex)
			{
				PicklistItem item = m_Picklist.Items[iIndex];
				...
				if (sFunction != null)
				{
					sHyperlinkStart = String.Format("<a href=\"javascript:{0}('{1}');\" class=\"pre\">", sFunction, iIndex);
					sHyperlinkEnd = "</a>";
				}
