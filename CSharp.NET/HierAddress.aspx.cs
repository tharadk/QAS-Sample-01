...
namespace com.qas.prowebintegration
{
	...
	public partial class HierAddress : HierBasePage
	{
		...
		protected void FormatAddress(ref string[] asLabels, ref string[] asLines)
		{
			...
			if (StoredRoute.Equals(Constants.Routes.Okay))
			{
				try
				{
					...
					FormattedAddress objAddress = searchService.GetFormattedAddress(StoredMoniker, GetLayout(), GetRequestTag());
                    SetDPVInfo((Constants.DPVStatus)Enum.Parse(typeof(Constants.DPVStatus), objAddress.DPVStatus.ToString()));   
					AddressLine[] aLines = objAddress.AddressLines;
		...
		protected void DisplayAddress(string[] asLabels, string[] asLines)
		{
			...
			// Add country row
			HtmlInputText InputCountry = AddAddressLine("Country or Datamap", StoredCountryName);
