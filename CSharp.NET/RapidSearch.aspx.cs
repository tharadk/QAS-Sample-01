	...
	public partial class RapidSearch : RapidBasePage
	{
		...
		override protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            if ( StoredDataMapList == null )
            {
                try
                {
                    ...
                    {
                        GoErrorPage(Constants.Routes.PreSearchFailed, "No available datasets");
                    }
		...
		protected void InitialSearch()
		{
			...
			try
			{
				...
				theQuickAddress.Engine = SearchEngine;
                StoredSearchEngine = SearchEngine;
                theQuickAddress.Flatten = false;

				result = theQuickAddress.CanSearch(DataID, GetLayout());

				if (result.IsOk)
				{
					// Get initial prompt
					PromptSet prompts = theQuickAddress.GetPromptSet(DataID, PromptSet.Types.Default);
		...
		protected void AutoStep(ref bool bFinalAddress, ref StepinWarnings eWarn)
		{
			...
			try
			{
				...
				while (m_Picklist.IsAutoStepinSafe || m_Picklist.IsAutoStepinPastClose)
				{
					...
					m_Picklist = theQuickAddress.StepIn(Moniker);
				}
		...
		private QuickAddress.EngineTypes SearchEngine
		{
			get
			{
                if (RadioSingleline.Checked) { return QuickAddress.EngineTypes.Singleline; }
                if (RadioTypedown.Checked)   { return QuickAddress.EngineTypes.Typedown; }
                if (RadioKeyfinder.Checked)  { return QuickAddress.EngineTypes.Keyfinder; }
